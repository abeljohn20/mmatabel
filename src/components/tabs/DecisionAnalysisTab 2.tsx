...
function Badge({ text, bg, border, color }: { text: string; bg: string; border: string; color: string }) {
  return (
    <div className="flex items-center justify-center px-2 py-0.5 rounded border" style={{ backgroundColor: bg, borderColor: border }}>
      <span className="text-xs font-medium" style={{ color, fontFamily: "var(--font-dm-sans)" }}>{text}</span>
    </div>
  );
}
